...
select @RowCnt = 1
declare @grptable Table (rownum int IDENTITY (1, 1) Primary key NOT NULL,RowData nvarchar(max))
...
cast('1' as varchar(max))+',cast('''+cast(getutcdate() as varchar(max)) + ''' as datetime)'+',cast('''+cast(getutcdate() as varchar(max)) +'''as datetime),'+'1'+','+'1'+','+cast('0' as varchar(max))+');'
  FROM [DBS99.COYOTEPOS.COM.AU].[AASandBox].[dbo].[CODETBL] where [CODE_KEY_TYPE] like 'GROUP'
  
   
select @MaxRows=count(*) from @grptable
...
begin
    select @ExecSql = ' ' + RowData + '' from @grptable where rownum = @RowCnt 
    --print @ExecSql
